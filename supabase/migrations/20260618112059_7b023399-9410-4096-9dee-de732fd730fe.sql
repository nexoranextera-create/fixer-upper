...
DECLARE r record; ins int := 0; recent_abs int; avg_eng numeric; trend numeric;
BEGIN
  FOR r IN SELECT id, full_name FROM public.profiles LOOP
    SELECT COUNT(*) INTO recent_abs FROM public.absences WHERE employee_id = r.id AND start_date > (CURRENT_DATE - INTERVAL '30 days');
    SELECT AVG(score) INTO avg_eng FROM public.engagement WHERE employee_id = r.id AND measured_at > (CURRENT_DATE - INTERVAL '30 days');
    ...
    IF recent_abs >= 3 AND COALESCE(avg_eng, 100) < 55 THEN
      ...
      SELECT r.id, 'critical', 'Burnout risk detected',
    ...
    ELSIF trend < -10 THEN
      INSERT INTO public.alerts (target_id, severity, title, description)
      SELECT r.id, 'warning', 'Engagement decline',
...
('Data privacy and GDPR', 'compliance', ARRAY['gdpr','privacy','data'], 'Personal data is processed only for legitimate HR purposes. Collaborators have the right to access, rectify, and request erasure of their data. AI assistant conversations are anonymised before being stored in audit logs.', 'en', 'all')
